...
create_mnemonic() {
  ...
  if ! kubectl get -n "$NAME" secrets/hermesnode-mnemonic >/dev/null 2>&1; then
    ...
    mnemonic=$(kubectl run -n "$NAME" -it --rm mnemonic --image=576263512135.dkr.ecr.ap-south-1.amazonaws.com/hermes/hermes-node:testnet-1.89.0_51 --restart=Never --command -- generate | grep MASTER_MNEMONIC | cut -d '=' -f 2 | tr -d '\r')
